
import React, { Component } from 'react';
import { Rate } from 'antd';
import PropTypes from 'prop-types';
import MovieService from '../../movie-service/movie-service';
import { SessionConsumer } from '../session/session';


class Card extends Component {

   /* eslint-disable */

  state = {
    rate : 0,
    borderStyle : {border: `2px solid #E90000`},
    value:this.props.cardInfo.rating,
  }

  genres = []

  serv = new MovieService()
  
  sessionId = null;
   /* eslint-enable */ 

  componentDidMount() {
  const {cardInfo} = this.props  
  const {id} = cardInfo   
  this.serv.getFilmInfo(id).then((res)=>{
      this.setState({rate:res.vote_average})
      const {rate} = this.state 
     if(res.genres.length!==0){
      this.genres = res.genres.reduce((ac,current) => {
       ac.push(current.name)
       return ac
      },[])}
      let color = '#E90000';
      if (rate>3 && rate<5) {
      color = '#E97E00'
      }
      if(rate>=5 && rate<=7 ) {
      color = '#E9D100'
      }  
      if (rate>7) {
      color= '#66E900'
     }
      const border =`2px solid ${color}`
       this.setState({borderStyle: {border}}) 
    })  
  }

  componentDidUpdate(prevProps,prevState) {
  const {value} = this.state
  if(prevState.value!==value){  
  const {cardInfo} = this.props
  const {id} = cardInfo
  if (value!==0 && typeof value !== "undefined") {
    this.serv.postRate(id,this.sessionId,{value})
    }
   }
  }
  
  handleChange = value => {
    this.setState({value });
  };

  render () {
  const {cardInfo} = this.props  
  let url;
  const {poster,id,title,releazeDate,text} = cardInfo 
  const {borderStyle,rate,value} = this.state
  if (!poster) {
      url = 'data:image/png;base64,';
      } 
  else {
    url=`https://image.tmdb.org/t/p/original/${poster}`
  }     
  const cardGenres = this.genres.map((item) =><li className="card__genre">{item}</li>)
  return (
    <SessionConsumer >
     {(sessionId) =>  {
       this.sessionId = sessionId
    return (    
    <li className="main__card card" key={id} id={id}>
      <img className="card__img" src={url} alt="poster not found"/>
        <div className="info">
        <span className="card__title">{title}</span>
        <span className="card__rate" style={borderStyle}><span>{rate}</span></span>
        </div>
        <span className="card__date">{releazeDate}</span>
        <ul className="card__genre-list">{cardGenres}</ul>
        <div className='card__text'>{text}</div>
      <Rate allowHalf defaultValue={0} count={10} style={{fontSize:'12px',
        position:'absolute',right:'12px',bottom:'12px'}} value={value} onChange={this.handleChange} />
    </li>
     )}
    }
    </SessionConsumer>
  );
 }
};

export default Card;
Card.defaultProps = {
  cardInfo:{}
};
Card.propTypes = {
  cardInfo:PropTypes.shape({
    title:PropTypes.string,
    genreList:PropTypes.array, // eslint-disable-line
    text:PropTypes.string,
    poster:PropTypes.string,
    releazeDate:PropTypes.any, // eslint-disable-line
    id:PropTypes.number
  }),
}

